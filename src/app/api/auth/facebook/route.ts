...
export async function POST(request: NextRequest) {
  try {
    ...
    const { facebookId, email, fullName, picture } = body;

    console.log('[AUTH-FACEBOOK] Facebook OAuth callback for email:', email);

    // Validate required fields
    if (!facebookId || !email || !fullName) {
    ...
    if (!user) {
      ...
      console.log('[AUTH-FACEBOOK] Created new user via Facebook OAuth:', user._id);
    } else if (user.provider !== 'facebook') {
      console.log('[AUTH-FACEBOOK] User exists with different provider:', user.provider);
    }
    ...
    console.log('[AUTH-FACEBOOK] Facebook login successful for user:', user._id, user.email);
